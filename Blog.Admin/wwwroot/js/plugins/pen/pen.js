...
(function(root, doc) {
   ...
   * 判断对象类型
   * @param {any} obj 对象
   * @param {any} type 类型名 Object,Array...
   * @returns 
   */
  ...
  utils.merge = function(config) {
    ...
    var defaults = {
      ...
      editor:null,//编辑器,DOM对象
      match:null,//编辑器对象id选择器
      placeholder:null,//
      saveAsMarkdown:true//
    };
   ...
   * 判断编辑器是否为空
   * 
   * @param {any} node 编辑器dom对象,默认从config中取
   * @returns 
   */
  ...
  /**
   * 
   * document.createRang() or Selection.getRangeAt 获得Range对象
   * Rang对象可以用于保存dom与被选中的文本
   * @returns 
   */
  Pen.prototype.getRange = function() {
    ...
    if (!containsNode(editor, range.commonAncestorContainer)) {
      range.selectNodeContents(editor);//如果选中元素不包括编辑器,则把编辑器也选中
      range.collapse(false);
    }
    return range;
  };
